...
// Size of the msg struct and a character
#define MSG_CHAR_SIZE (sizeof(msg_t)+sizeof(char))
/*
...
int getchar(int *c)
{
    char msg_c[MSG_CHAR_SIZE];
    msg_t* msg = (msg_t*) msg_c;
    enter_critical(); // See readme
    mbox_recv(mbox,msg);
    leave_critical();
    *c = msg->body[0];
    return 1;
...
{
    int count = 0, free_space = 0;
    mbox_stat(mbox, &count, &free_space);
    if(free_space >= MSG_CHAR_SIZE) {
        char msg_c[MSG_CHAR_SIZE];
        msg_t * msg = (msg_t*) msg_c;
        msg->size = 1;
        msg->body[0] = c->character;
        mbox_send(mbox, msg);
    }
}
